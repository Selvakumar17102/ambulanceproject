...
let totalCategory = Array()
let totalCategorySale = []
$.ajax({
    type: "POST",
    url: "ajax/map/categorySales.php",
    success: function(data){
      var obj = JSON.parse(data);
      obj.resp.map((resp) =>{
        totalCategorySale.push(parseInt(resp))
      })
    }
});
document.getElementById('tester').innerHTML = totalCategorySale
var d_2options1 = {
  chart: {
        height: 160,
        type: 'bar',
...
var options = {
    ...
    series: totalCategorySale,
    labels: ['totalCategory','sdg'],
    responsive: [{
        breakpoint: 1599,
        options: {
            chart: {
