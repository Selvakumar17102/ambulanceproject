		...
		if($responceData['status']){
            ...
            if(!empty($data->from_date) && !empty($data->to_date)){
                ...
                if($result->num_rows > 0){
                    $sql = "SELECT * FROM orders WHERE booking_date BETWEEN '$from_date' AND '$to_date' AND delivery_partner_id='$user_id' AND order_status=7";
                    $result = $conn->query($sql);
                    if($result->num_rows > 0){
                        $completed = $totalAmount = $clientAmount = $travelKm = 0;
                        $i = 0;
                        while($row = $result->fetch_assoc()){
                            ...
                            $output_array['GTS'][$i]['client_earning'] = (int)$row['amount_for_client'];
                            $output_array['GTS'][$i]['driver_earning'] = (int)($row['total_amount'] - $row['amount_for_client']);
                        ...
                        $output_array['overall_earning'] = (int)$totalAmount;
                        $output_array['completed'] = $completed;
                        $output_array['km_travelled'] = $travelKm;
                        $output_array['client_earning'] = $clientAmount;
                        $output_array['driver_earning'] = (int)($totalAmount - $clientAmount);
