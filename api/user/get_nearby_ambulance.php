<?php
    ...
    if(!empty($header['authorization'])){
		...
		if($responceData['status']){
            ...
            if(!empty($data->vehicle_type_id) && !empty($data->login_id)){
                ...
                if($result->num_rows > 0){
                    ...
                    $sql = "SELECT * FROM delivery_partner WHERE delivery_partner_branch_id='$login_id' AND vehicle_type='$vehicle_type_id' AND request_status=2 AND delivery_partner_online_status=1 AND delivery_partner_status=1 AND (emergency=0 OR emergency=2)";
                    $result = $conn->query($sql);
                    if($result->num_rows > 0){
                        $i = 0;
