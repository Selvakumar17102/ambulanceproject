		...
		if($responceData['status']){
			...
			header('authorization: ' . $responceToken);
			    ...
			    if(!empty($data->patient_name) && !empty($data->blood_group) && !empty($data->unit)){
                    ...
                    $sql = "INSERT INTO blood_request (user_id,patient_name,blood_group,request_date,unit,request_city_id,alter_phone_no,hospital_location,emergency_status,additional_notes,check_terms,dummy_otp) VALUES ('$user_id','$patient_name','$blood_group','$request_date','$unit','$city_id','$alter_phone_no','$location','$emergency_status','$additional_notes','$check_terms','$dummy_otp')";
