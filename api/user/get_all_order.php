		...
		if($responceData['status']){
            ...
            if(!empty($data->service_type)){
                ...
                if($result->num_rows > 0){
                    $sql = "SELECT * FROM orders WHERE user_id='$user_id' AND service_type='$service_type' ORDER BY order_id DESC";
                    $result = $conn->query($sql);
                    if($result->num_rows > 0){
                        $i = 0;
                        while($row = $result->fetch_assoc()){
                            ...
                            $output_array['GTS'][$i]['driver_name'] = $driverTable['delivery_partner_name'];
                            $output_array['GTS'][$i]['driver_mobile'] = $driverTable['delivery_partner_phone'];
                            $output_array['GTS'][$i]['driver_latitude'] = $driverTable['delivery_partner_latitude'];
                            $output_array['GTS'][$i]['driver_longitude'] = $driverTable['delivery_partner_longitude'];
                            $output_array['GTS'][$i]['driver_distance'] = $km.' Km';
                            $output_array['GTS'][$i]['ambulance_name'] = $driverTable['delivery_partner_vehicle_name'];
                            $output_array['GTS'][$i]['hospital_name'] = $driverTable['hospital_name'];
                            ...
                            // $output_array['GTS'][$i]['booking_time'] = date('h:i A', strtotime($row['booking_time']));
                            $i++;
                        }
