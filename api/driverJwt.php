<?php
	...
	function createToken($conn,$login_id){
		...
		$current_expire_claim = $issuedat_claim + 900;
	...
	function checkAndGenerateToken($conn,$token){
		...
		try {
			...
			if($e->getMessage() == 'Expired token'){
				...
				$sql = "SELECT * FROM delivery_partner WHERE delivery_partner_id='$user_id' AND refresh_token!=''";
				$result = $conn->query($sql);
				if($result->num_rows > 0){
					...
					try {
						...
						$sql = "UPDATE delivery_partner SET refresh_token='$new_refresh_token' WHERE delivery_partner_id='$user_id'";
						...
						$needle = 'handle token prior';

						if (strpos($haystack, $needle) !== false) {
							$res['status'] = 1;
							$res['msg'] = 'Please retry';
							$res['token'] = $token;
							$res['is_changed'] = false;
						} else{
							$res['status'] = 0;
